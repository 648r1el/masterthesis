...
class BasicParams:
    ...
    def initialize(self, parser):
        ...
        # Dataset parameters
        parser.add_argument('--omics-mode', type=str, default='a',
                            help='omics types would like to use in the model, options: [abc | ab | a | b | c | abcd]')
        parser.add_argument('--data-root', type=str, default='./data',
                            help='path to input data')
        parser.add_argument('--batch-size', type=int, default=32,
                            help='input data batch size')
        parser.add_argument('--label', type=str, default='metastasis',
                            help='The label by which the classification task is performed. Only applied when the downstream task is a classification')
        parser.add_argument('--num-threads', default=0, type=int,
                            help='number of threads for loading data')
        parser.add_argument('--set-pin-memory', action='store_true',
                            help='set pin_memory in the dataloader to increase data loading performance')
        parser.add_argument('--not-stratified', action='store_true',
                            help='do not apply the stratified mode in train/test split if set true')
        parser.add_argument('--use-sample-list', action='store_true',
        ...
        parser.add_argument('--down-task-file', type=str, default='clinical.tsv',
                            help='Tsv file containing the needed information for the downstream task')
        ...
        parser.add_argument('--net-VAE', type=str, default='fc_sep',
                            help='specify the backbone of the VAE, default is the one dimensional CNN, options: [conv_1d | fc_sep | fc]')
        parser.add_argument('--net-down', type=str, default='multi_FC_classifier',
        ...
        parser.add_argument('--filter-num', type=int, default=8,
                            help='number of filters in the last convolution layer in the generator')
        parser.add_argument('--conv-k-size', type=int, default=9,
        ...
        parser.add_argument('--latent-space-dim', type=int, default=128,
        ...
        parser.add_argument('--init-gain', type=float, default=0.02,
                            help='scaling factor for normal, xavier and orthogonal initialization methods')

        # Loss parameters
        parser.add_argument('--recon-loss', type=str, default='BCE',
                            help='chooses the reconstruction loss function, options: [BCE | MSE | L1 | NLL]')
        parser.add_argument('--reduction', type=str, default='mean',
                            help='chooses the reduction to apply to the loss function, options: [sum | mean]')
        parser.add_argument('--k-kl', type=float, default=0.01,
                            help='weight for the kl loss')
        parser.add_argument('--k-embed', type=float, default=0.001,
                            help='weight for the embedding loss')
        parser.add_argument('--loss-function', type=str, default='kl',
                            help='Loss function which shall be used, options [kl | mmd]')
        ...
        parser.add_argument('--epoch-to-load', type=str, default='latest',
                            help='the epoch number to load, set latest to load latest cached model')
        parser.add_argument('--experiment-to-load', type=str, default='test',
