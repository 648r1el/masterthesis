...
def load_file(param, file_name, row_index_name, sample_list):
    """
    ...
    if param.file_format == 'tsv':
        ...
        df = pl.read_csv(file_path, separator='\t', has_header=True, columns=[row_index_name] + sample_list)
    elif param.file_format == 'csv':
        file_path = os.path.join(param.data_root, file_name + '.csv')
        print('Loading data from ' + file_path)
        df = pd.read_csv(file_path, header=0, index_col=0, na_filter=param.detect_na)#, na_values=0)
