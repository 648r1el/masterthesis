...
class ABCDDataset(BasicDataset):
    """
    ...
    For lncRNA expression data, file should be prepared as '/path/to/data/D.tsv'.
    ...
    def __init__(self, param):
        ...
        if param.use_sample_list:
            ...
            self.sample_list = pd.read_csv(sample_list_path, delimiter='\t')['samples'].to_list()
        else:
            self.sample_list = None
        # Load data for A
        A_df = load_file(param, 'A', 'Gene', self.sample_list)
        self.sample_list = A_df.columns[1:]  # the first column is the feature column which shall be ignored
        ...
        if param.use_feature_lists:
            ...
            feature_list_A = A_df['Gene'].to_list()
        A_df = A_df.filter(pl.col('Gene').is_in(feature_list_A))
        self.A_dim = A_df.shape[0]
        self.sample_num = A_df.shape[1] - 1  # polars includes the feature names as column, not as index. This column must be subtracted from the shape 
        A_array = A_df.select(self.sample_list).to_numpy()
        ...
        # Load data for B
        B_df = load_file(param, 'B', 'Probe', self.sample_list)
        ...
        if param.use_feature_lists:
            ...
            feature_list_B = B_df['Probe'].to_list()
        B_df = B_df.filter(pl.col('Probe').is_in(feature_list_B)).fill_nan(0).fill_null(0)
        if param.ch_separate:
            ...
            B_array = B_df.select(self.sample_list).to_numpy()
            if self.param.add_channel:
                # Add one dimension for the channel
                B_array = B_array[np.newaxis, :, :]
        ...
        # Load data for C
        C_df = load_file(param, 'C', 'miRNA_ID', self.sample_list)
        ...
        if param.use_feature_lists:
            ...
            feature_list_C = C_df['miRNA_ID'].to_list()
        C_df = C_df.filter(pl.col('miRNA_ID').is_in(feature_list_C))
        self.C_dim = C_df.shape[0]
        C_array = C_df.select(self.sample_list).to_numpy()
        if self.param.add_channel:
            # Add one dimension for the channel
        ...
        # Load data for D
        D_df = load_file(param, 'D', 'Gene', self.sample_list)
        # Get the feature list for D
        if param.use_feature_lists:
            feature_list_D_path = os.path.join(param.data_root, 'feature_list_D.tsv')  # get the path of feature list
            feature_list_D = np.loadtxt(feature_list_D_path, delimiter='\t', dtype='<U32')
        else:
            feature_list_D = D_df['Gene'].to_list()
        D_df = D_df.filter(pl.col('Gene').is_in(feature_list_D))
        self.D_dim = D_df.shape[0]
        D_array = D_df.select(self.sample_list).to_numpy()
        if self.param.add_channel:
            # Ddd one dimension for the channel
            D_array = D_array[np.newaxis, :, :]
        self.D_tensor_all = torch.Tensor(D_array)
        self.omics_dims.append(self.D_dim)

        self.sample_list = pd.Series(self.sample_list)

        self.class_num = 0
        if param.downstream_task == 'classification':
            # Load labels
            labels_path = os.path.join(param.data_root, param.down_task_file)       # get the path of the label
            labels_df = pd.read_csv(labels_path, sep='\t', header=0, index_col=0).loc[self.sample_list, :]
            self.labels_array = labels_df.loc[:, param.label].values
            # Get the class number
            self.class_num = len(labels_df.loc[:, param.label].unique())
            ...
            # Load survival data
            survival_path = os.path.join(param.data_root, 'clinical.tsv')   # get the path of the survival data
            survival_df = pd.read_csv(survival_path, sep='\t', header=0, index_col=0).loc[self.sample_list, :]
            self.survival_T_array = survival_df.loc[:, 'survival_time'].astype(float).values
            self.survival_E_array = survival_df.loc[:, 'vital_status'].values
    ...
    def __getitem__(self, index):
        ...
        # Get the tensor of D
        if self.param.add_channel:
            D_tensor = self.D_tensor_all[:, :, index]
        else:
            D_tensor = self.D_tensor_all[:, index]
        ...
        if self.param.downstream_task == 'classification':
            ...
            return {'input_omics': [A_tensor, B_tensor, C_tensor, D_tensor], 'label': label, 'index': index}
        elif self.param.downstream_task == 'regression':
            # Get target value
            value = self.values_array[index]
            return {'input_omics': [A_tensor, B_tensor, C_tensor, D_tensor], 'value': value, 'index': index}
            ...
            return {'input_omics': [A_tensor, B_tensor, C_tensor, D_tensor], 'survival_T': survival_T, 'survival_E': survival_E, 'y_true': y_true, 'index': index}
            ...
            survival_E = self.survival_E_array[index]
            y_true = self.y_true_tensor[index, :]
            return {'input_omics': [A_tensor, B_tensor, C_tensor, D_tensor], 'label': label, 'value': value,
            ...
            return {'input_omics': [A_tensor, B_tensor, C_tensor, D_tensor], 'index': index}
