...
class ABCDataset(BasicDataset):
    ...
    def __init__(self, param):
        ...
        if param.use_sample_list:
            ...
            self.sample_list = pd.read_csv(sample_list_path, delimiter='\t')['samples']
        else:
        ...
        B_df = B_df.loc[feature_list_B, self.sample_list].fillna(0)
        ...
        if param.downstream_task == 'classification':
            # Load labels
            labels_path = os.path.join(param.data_root, 'clinical.tsv')     # get the path of the label
            labels_df = pd.read_csv(labels_path, sep='\t', header=0, index_col=0).loc[self.sample_list, :]
            self.labels_array = labels_df.loc[:, 'stage'].values
            # Get the class number
            self.class_num = len(labels_df.loc[:, 'stage'].unique())
