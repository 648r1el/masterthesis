...
class FCBlock(nn.Module):
    ...
    """
    def __init__(self, input_dim, output_dim, norm_layer=nn.BatchNorm1d, leaky_slope=0.2, dropout_p=0, activation=True, normalization=True, activation_name='LeakyReLU', leaky_relu_inplace=True):
        """
        ...
        if activation:
            if activation_name.lower() == 'leakyrelu':
                self.fc_block.append(nn.LeakyReLU(negative_slope=leaky_slope, inplace=leaky_relu_inplace))
...
class ConvVaeABCD(nn.Module):
    ...
    def __init__(self, omics_dims, norm_layer=nn.InstanceNorm1d, filter_num=8, kernel_size=9, leaky_slope=0.2,
                 dropout_p=0, ratio_1B=16, ratio_2B=16, ratio_1A=4, ratio_2A=4, ratio_1C=2, ratio_2C=2, ratio_1D=2,
                 ratio_2D=2, ratio_3=16, latent_dim=256):
        """
        ...
        hidden_dim_1 = (B_dim // ratio_1B // ratio_2B + A_dim // ratio_1A // ratio_2A + C_dim // ratio_1C // ratio_2C + D_dim // ratio_1D // ratio_2D) // ratio_3 * filter_num * 4
        ...
        self.narrow_C = hidden_dim_2 // (4 * filter_num) * ratio_3 * (C_dim // ratio_1C // ratio_2C) // (
                B_dim // ratio_1B // ratio_2B + A_dim // ratio_1A // ratio_2A + C_dim // ratio_1C // ratio_2C)
        self.narrow_D = hidden_dim_2 // (4 * filter_num) * ratio_3 * (D_dim // ratio_1D // ratio_2D) // (
                B_dim // ratio_1B // ratio_2B + A_dim // ratio_1A // ratio_2A + C_dim // ratio_1C // ratio_2C + D_dim // ratio_1D // ratio_2D)
        self.B_dim = B_dim
        self.A_dim = A_dim
        self.C_dim = C_dim
        self.D_dim = D_dim
        ...
        self.down_sample_2C = DownSample(filter_num, filter_num * 2, down_ratio=ratio_2C, kernel_size=kernel_size,
                                         norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p)
        # D 1 -> 8
        self.down_sample_1D = DownSample(1, filter_num, down_ratio=ratio_1D, kernel_size=kernel_size,
                                         norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=0)
        # D 8 -> 16
        self.down_sample_2D = DownSample(filter_num, filter_num * 2, down_ratio=ratio_2D, kernel_size=kernel_size,
                                         norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p)
        ...
        # D 16 -> 8
        self.up_sample_2D = UpSample(filter_num * 2, filter_num, up_ratio=ratio_2D, kernel_size=kernel_size,
                                     norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p)
        # D 8 -> 1
        self.up_sample_3D = UpSample(filter_num, filter_num, up_ratio=ratio_1D, kernel_size=kernel_size,
                                     norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=0)
        # D Output
        self.output_conv_D = OutputConv(filter_num, 1)

    def encode(self, x):
        level_2_B = self.down_sample_1B(x[1])
        level_2_A = self.down_sample_1A(x[0])
        level_2_C = self.down_sample_1C(x[2])
        level_2_D = self.down_sample_1D(x[3])

        level_3_B = self.down_sample_2B(level_2_B)
        level_3_A = self.down_sample_2A(level_2_A)
        level_3_C = self.down_sample_2C(level_2_C)
        level_3_D = self.down_sample_2D(level_2_D)

        level_3 = torch.cat((level_3_B, level_3_A, level_3_C, level_3_D), 2)
...
class FcSepVaeABCD(nn.Module):
    ...
    def __init__(self, omics_dims, norm_layer=nn.BatchNorm1d, leaky_slope=0.2, dropout_p=0, dim_1B=128, dim_2B=1024,
                 dim_1A=2048, dim_2A=1024, dim_1C=1024, dim_2C=1024, dim_1D=1024, dim_2D=1024, dim_3=512, latent_dim=256):
        """
        ...
        self.encode_fc_1C = FCBlock(self.C_dim, dim_1C, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                    activation=True)
        self.encode_fc_1D = FCBlock(self.D_dim, dim_1D, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                    activation=True)
        ...
        self.encode_fc_2C = FCBlock(dim_1C, dim_2C, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                    activation=True)
        self.encode_fc_2D = FCBlock(dim_1D, dim_2D, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                    activation=True)

        # Layer 3
        self.encode_fc_3 = FCBlock(dim_2B+dim_2A+dim_2C+dim_2D, dim_3, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                   activation=True)
        ...
        # Layer 2
        self.decode_fc_2 = FCBlock(dim_3, dim_2B+dim_2A+dim_2C+dim_2D, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                   activation=True)
        ...
        self.decode_fc_3C = FCBlock(dim_2C, dim_1C, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                    activation=True)
        self.decode_fc_3D = FCBlock(dim_2D, dim_1D, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                    activation=True)
        ...
        self.decode_fc_4C = FCBlock(dim_1C, self.C_dim, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=0,
                                    activation=False, normalization=False)
        self.decode_fc_4D = FCBlock(dim_1D, self.D_dim, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=0,
                                    activation=False, normalization=False)
    ...
    def encode(self, x):
        ...
        level_2_D = self.encode_fc_1D(x[3])

        level_3_B = self.encode_fc_2B(level_2_B)
        level_3_A = self.encode_fc_2A(level_2_A)
        level_3_C = self.encode_fc_2C(level_2_C)
        level_3_D = self.encode_fc_2D(level_2_D)
        level_3 = torch.cat((level_3_B, level_3_A, level_3_C, level_3_D), 1)
    ...
    def decode(self, z):
        ...
        level_2_D = level_2.narrow(1, self.dim_2B+self.dim_2A+self.dim_2C, self.dim_2D)
        ...
        level_3_D = self.decode_fc_3D(level_2_D)
        ...
        recon_D = self.decode_fc_4D(level_3_D)

        return [recon_A, recon_B_list, recon_C, recon_D]
...
class FcVaeABCD(nn.Module):
    ...
    def __init__(self, omics_dims, norm_layer=nn.BatchNorm1d, leaky_slope=0.2, dropout_p=0, dim_1B=384, dim_2B=256,
                 dim_1A=384, dim_2A=256, dim_1C=384, dim_2C=256, dim_1D=384, dim_2D=256, dim_3=256, latent_dim=256):
        """
            Construct a fully-connected variational autoencoder
            Parameters:
                omics_dims (list)       -- the list of input omics dimensions
...
class FcVaeA(nn.Module):
    ...
    def __init__(self, omics_dims, norm_layer=nn.BatchNorm1d, leaky_slope=0.2, dropout_p=0, dim_1A=1024, dim_2A=1024,
                 dim_3=512, latent_dim=256, leaky_relu_inplace=True):
        """
            Construct a fully-connected variational autoencoder
            Parameters:
                omics_dims (list)       -- the list of input omics dimensions
        ...
        # Layer 1
        self.encode_fc_1A = FCBlock(self.A_dim, dim_1A, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                    activation=True, leaky_relu_inplace=leaky_relu_inplace)
        # Layer 2
        self.encode_fc_2A = FCBlock(dim_1A, dim_2A, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                    activation=True, leaky_relu_inplace=leaky_relu_inplace)
        # Layer 3
        ...
        # Layer 4
        self.encode_fc_mean = FCBlock(dim_3, latent_dim, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=0,
                                      activation=False, normalization=False, leaky_relu_inplace=leaky_relu_inplace)
        self.encode_fc_log_var = FCBlock(dim_3, latent_dim, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=0,
                                         activation=False, normalization=False, leaky_relu_inplace=leaky_relu_inplace)
...
class FcVaeD(nn.Module):
    """
        Defines a fully-connected variational autoencoder for multi-omics dataset
    """
    def __init__(self, omics_dims, norm_layer=nn.BatchNorm1d, leaky_slope=0.2, dropout_p=0, dim_1D=1024, dim_2D=1024, dim_3=512, latent_dim=256):
        """
            Donstruct a fully-connected variational autoencoder
            Parameters:
                omics_dims (list)       -- the list of input omics dimensions
                norm_layer              -- normalization layer
                leaky_slope (float)     -- the negative slope of the Leaky ReLU activation function
        ...
        self.encode_fc_2D = FCBlock(dim_1D, dim_2D, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                    activation=True)
        # Layer 3
        self.encode_fc_3 = FCBlock(dim_2D, dim_3, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                   activation=True)
        # Layer 4
        self.encode_fc_mean = FCBlock(dim_3, latent_dim, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=0,
                                      activation=False, normalization=False)
        ...
        self.decode_fc_z = FCBlock(latent_dim, dim_3, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                   activation=True)
        # Layer 2
        self.decode_fc_2 = FCBlock(dim_3, dim_2D, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                   activation=True)
        # Layer 3
        self.decode_fc_3D = FCBlock(dim_2D, dim_1D, norm_layer=norm_layer, leaky_slope=leaky_slope, dropout_p=dropout_p,
                                    activation=True)
    ...
    def encode(self, x):
        ...
        level_4 = self.encode_fc_3(level_3_D)
    ...
    def decode(self, z):
        ...
        level_3_D = self.decode_fc_3D(level_2)

        recon_D = self.decode_fc_4D(level_3_D)

        return [None, None, None, recon_D]
...
class OmiEmbed(nn.Module):
    ...
    def __init__(self, net_VAE, net_down, omics_dims, omics_mode='multi_omics', norm_layer=nn.InstanceNorm1d, filter_num=8, kernel_size=9,
        ...
        if net_VAE == 'conv_1d':
            ...
            elif omics_mode == 'abcd':
                self.vae = ConvVaeABCD(omics_dims, norm_layer, filter_num, kernel_size, leaky_slope, dropout_p, latent_dim=latent_dim)
            ...
            if omics_mode == 'abc':
                ...
                self.vae = FcVaeABCD(omics_dims, norm_layer, leaky_slope, dropout_p, latent_dim=latent_dim)
...
def define_VAE(net_VAE, omics_dims, omics_mode='multi_omics', norm_type='batch', filter_num=8, kernel_size=9, leaky_slope=0.2, dropout_p=0,
    ...
    if net_VAE == 'conv_1d':
        if omics_mode == 'abc':
            ...
            net = ConvVaeABCD(omics_dims, norm_layer, filter_num, kernel_size, leaky_slope, dropout_p,
                              latent_dim=latent_dim)
        ...
        if omics_mode == 'abc':
            ...
            net = FcVaeABCD(omics_dims, norm_layer, leaky_slope, dropout_p, dim_3=1024, latent_dim=latent_dim)
        ...
        if omics_mode == 'abc':
            ...
            net = FcVaeD(omics_dims, norm_layer, leaky_slope, dropout_p, latent_dim=latent_dim)
        elif omics_mode == 'abcd':
            net = FcVaeABCD(omics_dims, norm_layer, leaky_slope, dropout_p,
                            dim_1A=2048, dim_1B= 2048, dim_1C=2048, dim_1D=2048,
                            dim_2A=1024, dim_2B= 1024, dim_2C=1024, dim_2D=1024,
                            dim_3=512, latent_dim=latent_dim)
    else:
        raise NotImplementedError('VAE model name [%s] is not recognized' % net_VAE)
